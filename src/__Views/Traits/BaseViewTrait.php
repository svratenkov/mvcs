...
{
	/**
	 * @var	array	$data	View data - associative [key => value] array
	 */
	protected $data;

	/**
	 * @var mixed	$template	View template of any kind compatible to renderer
	 */
	protected $template;
...
//	public $renderer;
	...
	{
		$this->setTemplate($template)->setData($data)->setRenderer($renderer);
	}
	...
	//	$data = $this->resolveData();
		...
		// Get renderer and render this view
	//	$renderer = $this->resolveRenderer();
		$renderer = $this->getRenderer();
		$content = $renderer($this->getTemplate(), $data);
	...
	{
		try {
			return $this->render(FALSE);	// don't render child views
		}
		catch(\Exception $e) {
